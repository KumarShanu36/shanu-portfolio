...
import { auth, loginWithGoogle, logout, db, hasFirebaseConfig } from "../firebase";
    ...
    <div className="bg-zinc-900 border border-gray-700 p-6 rounded-xl shadow-lg max-w-xl mx-auto mt-5">
      ...
      <div className="h-72 overflow-y-auto border border-gray-700 p-3 rounded-lg bg-zinc-800 mb-4 space-y-3">
        {messages.length === 0 && (
          <p className="text-center text-sm text-gray-400">
            {hasFirebaseConfig ? "Belum ada pesan" : "Firebase chat belum dikonfigurasi."}
          </p>
        )}
        {messages.map((msg) => (
          <div
      ...
      {!hasFirebaseConfig ? (
        <div className="rounded-lg border border-amber-500/40 bg-amber-500/10 p-4 text-sm text-amber-100">
          Firebase chat is disabled locally. Set the `VITE_FIREBASE_*` variables to enable Google login and real-time messages.
        </div>
      ) : user ? (
